...
uint64_t entry(void *ctx, struct standard_metadata *std_meta){
    ...
    parse_tcp: {
        ...
        headers.tcp.res = (uint8_t)((load_byte(pkt, BYTES(packetOffsetInBits))) & BPF_MASK(uint8_t, 4));
        packetOffsetInBits += 4;

        headers.tcp.flags = (uint8_t)((load_byte(pkt, BYTES(packetOffsetInBits))));
        packetOffsetInBits += 8;
    ...
    accept:
    {
        uint64_t vm_info_0 = 0;
        {

            if (headers.ipv4.protocol == 6) {
                if ((headers.tcp.flags & 0x10) == 0) {
                    if ((headers.tcp.flags & 0x2) != 0) {

                        vm_info_0 = get_vm_info();
;
                        if (((uint32_t)(vm_info_0 >> 0x20) < (uint32_t)vm_info_0)) {
                            pass = false;
                        }
